-- 005: restore the basis gating, and say when a missing category is the cause.
--
-- ⚠️ FIXING MY OWN REGRESSION. Migration 003 was built by editing a dump of the
-- function taken BEFORE 002 was applied, so applying it silently reverted the
-- basis gating. Nothing caught it: the check for 003 was "did any score move",
-- and basis gating is inert on today's data — so removing it moved nothing
-- either. "Nothing moved" was necessary and nowhere near sufficient.
--
-- The lesson, recorded because it will happen again otherwise: when editing a
-- stored function, ALWAYS dump it fresh immediately before editing, and verify
-- afterwards that every earlier change is still present by name, not by
-- behaviour.
--
-- This migration is built from the live definition and restores basis gating
-- alongside the two changes that survived (basket sums, and the figure test).
-- It also adds 'category_not_disclosed', because after 004 a company that stops
-- disclosing a category was being told its data was too old.

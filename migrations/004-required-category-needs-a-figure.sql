-- 004: a category in the basket must have a figure, or the year is not scorable.
--
-- Completes Rule 1. A category that still applies and has stopped being disclosed
-- is a GAP, not a basket change — the year should not be scored rather than the
-- missing figure being treated as zero.
--
-- Nothing today has a required category without a figure, so no company should
-- move. That only becomes possible once relevance comes from a company's business
-- model rather than its sector, which is why this goes in before the scrape worker

-- 039: a year's reporting basis includes its scope 3 readings, not only scope 1 and 2.
--
-- Found 2026-09-02 while proving migration 038. score_company built each year's basis from that
-- year's scope 1 and 2 row and nothing else, so a break confined to scope 3 was recorded, applied
-- to the readings by apply_basis_break, and then had no effect on the score at all.
--
-- It had never shown up because the gate had never fired: every row in the database was basis 1
-- from the day the column was added until H&M's Sellpy break was applied this afternoon. A check
-- with nothing to check passes, and looks identical to one that works.
--
-- Regenerated from the live function body with only the window-basis block changed, rather than
-- retyped, so the other 900 lines are provably identical to what was running.
--
-- Expected: every current score unchanged. H&M is on basis 2 throughout from the 2024 report, the
-- other three are entirely basis 1, so no company straddles a break today.
  ...
  FROM (
    ...
    SELECT DISTINCT ON (s12.year) s12.year, s12.s12_status,
      (
        ...
        FROM (
          SELECT s12.basis_id::text AS b
          UNION
          SELECT w.basis_id::text
          FROM (
            SELECT DISTINCT ON (s3.category) s3.basis_id
            FROM scope3 s3
...
-- Verified 2026-09-02, three ways, the last two inside transactions that were rolled back:
--
--   1. All four companies rescored. Every score identical, H&M still 1.5095032111311140925670.
--      No company straddles a break today, so the change had to be inert and was.
--   2. Scope 3 only: Microsoft's readings from the 2025 publication set to basis 2, scope 1 and 2
--      untouched. score_status 'unknown', unknown_reason 'basis_change'. This is the case that was
--      silently ignored before today and is the reason the migration exists.
--   3. Scope 1 and 2: the earlier test still behaves. A break at Microsoft's 2024 publication still
--      refuses the run, so the path that did work has not been traded for the one that did not.

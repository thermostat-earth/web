-- 045: when EVERY year is knocked out by a missing category, say so.
--
-- Found by previewing H&M's release on 2026-09-03. H&M would publish as
-- "unknown — no_scope12_data" while holding five years of scope 1 and 2, which is
-- not merely unhelpful: it is false, and it points anyone investigating at the one
-- part of the disclosure that is complete.
--
-- The real cause is category 2. H&M reports capital goods as N/A in every year, the
-- category applies to it, and FIND VALID YEARS requires every category in the basket
-- to carry a figure. So all six years are rejected and the window comes back empty.
--
-- score_company already knows how to say this. The block that picks an unknown_reason
-- carries the comment "name the cause, not the symptom" and chooses
-- 'category_not_disclosed' from v_years_missing_cat. It was simply unreachable here:
-- it sits AFTER the empty-window early exit, so it runs when a missing category costs
-- the company SOME of its years and never when it costs ALL of them — which is the
-- more serious case and the one it was written for.
--
-- No new vocabulary. 'category_not_disclosed' already exists and the public site
-- already renders it, so nothing new is published; a company simply stops being told
-- its scope 1 and 2 are missing when they are not.

create or replace function public.score_company_reason_for_empty_window(
  p_company_id text, p_years_missing_cat integer[])
returns text language sql immutable as $$
  -- Kept as its own function so the rule has one home and the next person changing it
  -- does not have to find it inside a thousand-line procedure.
  select case
    when p_years_missing_cat is not null and array_length(p_years_missing_cat, 1) > 0
      then 'category_not_disclosed'
    else 'no_scope12_data'
  end;
$$;

comment on function public.score_company_reason_for_empty_window(text, integer[]) is
  'Why a company has no scorable years at all: a category it does not disclose, or genuinely no scope 1 and 2 data.';

-- score_company, with the empty-window early exit now naming its real cause.
...
BEGIN
  ...
  IF v_window_years IS NULL OR ARRAY_LENGTH(v_window_years, 1) = 0 THEN
    -- Migration 045. This used to hard-code 'no_scope12_data', which told H&M its scope 1
    -- and 2 were missing while it held five years of them. The window was empty because
    -- category 2 has no figure in any year, and the block below that picks a reason --
    -- the one commented "name the cause, not the symptom" -- never runs when the window
    -- is empty. It handled a missing category costing SOME years and not ALL of them.
    v_unknown_reason := public.score_company_reason_for_empty_window(
      p_company_id, v_years_missing_cat);
    INSERT INTO company_scores_public (company_id, company_name, sector, country_hq, score_status, unknown_reason, last_updated)

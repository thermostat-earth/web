-- 023-score-company-by-publication.sql
-- score_company resolves each year to the most recently PUBLISHED reading.
--
-- Fourteen ORDER BY clauses change from "reporting_year DESC" to "pub_order DESC,
-- reporting_year DESC". Publication decides; reporting year only breaks a tie. Nothing else in
-- the function is touched.
--
-- Dry-run in a transaction across all four companies before applying: every score, status and
-- year range came back identical. That is the whole reason to do it now — the two orderings
-- agree on every figure we currently hold, so this is a correctness change made while it is
-- provably a no-op, rather than after a document arrives out of order and quietly moves a
-- public number.
--
-- Before: Chanel 4.0000/4.0000 2021-2024 · H&M 1.5095/1.5206 2022-2024
--         ITV 1.4528/1.4589 2021-2024 · Microsoft 4.0000/4.0000 2020-2024
-- After:  identical.
...
BEGIN
  ...
  SELECT ARRAY_AGG(sub.year ORDER BY sub.year),
         ARRAY_AGG(sub.basis_id ORDER BY sub.year)
    INTO v_window_years, v_window_basis
  FROM (
    SELECT DISTINCT ON (s12.year) s12.year, s12.s12_status, s12.basis_id
    FROM scope12 s12

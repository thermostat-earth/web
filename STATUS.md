...
## Recently done
- **Why? + About consolidated into one /about page (2026-07-13):** mission statement, the problem, what we do, what makes it different, principles (the PRD non-negotiables), and "who's behind it" — named-founder (Felix Edge-Partington, name + one-line role) + funding line (independent, takes no money from scored companies, self-funded by founder for now, non-profit entity once the model is proven). Nav + footer now show a single **About**; old `/why` 307-redirects to `/about`. Copy pulled from Notion PRD v0.4 §1/§3/§6.

...
## Done this session (Scores dashboard cards)
- Scores page now has **two views**: **Dashboard** (default, cards) and **Thermometer** (one centred tube, companies off both sides). View-aware subtitle.
...
1. **Thermometer view** — bring to the same standard as the cards.
2. **Why?** + **About** page content (About = named-founder framing + funding line).
3. Delete temp preview pages: `/logos`, `/score-layouts`.
4. Phone/tablet responsive pass.
5. **Full legal review** + disclaimers / Terms / corrections.
6. **QA the 4 companies** (random checks vs sources; clear any internal NEEDS REVIEW; fix ITV 2025 dup row, Chanel missing 2024 source, Microsoft 2024 boundary check, Cat-11-materiality-for-fashion question; **Microsoft "≈ sector average" vs off-scale dot** — the `above_max` flag and the score/median disagree, check the data).
7. **Soft launch:** point thermostat.earth (framed "early build, more weekly").
